 ...
 * @since 2.2.0
 */
public class DatasourceBothHealthCheckTest extends Arquillian {
    ...
    @Test
    @RunAsClient
    public void livenessHealthApiShouldReturnUp() throws IOException {

        JsonObject healthApiResponse = getHealthApiResponse("/health/live");
        Assert.assertNotNull(healthApiResponse);
        JsonArray checks = healthApiResponse.getJsonArray("checks");
        Assert.assertEquals(checks.size(), 1);
        Assert.assertEquals("DataSourceHealthCheck", checks.get(0).asJsonObject().getString("name"));
    ...
    @Test
    @RunAsClient
    public void readinessHealthApiShouldReturnUp() throws IOException {

        JsonObject healthApiResponse = getHealthApiResponse("/health/ready");
        Assert.assertNotNull(healthApiResponse);
        JsonArray checks = healthApiResponse.getJsonArray("checks");
        Assert.assertEquals(checks.size(), 1);
        Assert.assertEquals("DataSourceHealthCheck", checks.get(0).asJsonObject().getString("name"));
